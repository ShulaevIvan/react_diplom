  ...
  const loadMoreHandler = async () => {
    ...
    if (context.state.activeCategory === 'all') {
      ...
      const allFetch = async () => {
        context.state.allCategories.forEach(async (item) => {
          console.log(item.id)
          await fetch(`http://localhost:7070/api/items?categoryId=${item.id}&offset=6`)
            ...
            setTimeout(() => {
              context.setState(prevState => ({
                ...
                goodsCategories: prevState.goodsCategories =  [
                 ...
                 ...allData.filter(({ id: dataId }) => !context.state.goodsCategories.some(({ id: curretnId }) => curretnId === dataId)),
                ],
